...
public class CwCommentBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cw/comment/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CwCommentBean[] beans=mapper.convertValue(obj, CwCommentBean[].class);
	...
	// @Override
	public Result addOne(final CwCommentBean entity) {
	...
	// @Override
	public Result selectList(final CwCommentQuery query) {
	...
	// @Override
	public Result editOne(final CwCommentBean entity) {
	...
	public Result editSelective(final CwCommentBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
