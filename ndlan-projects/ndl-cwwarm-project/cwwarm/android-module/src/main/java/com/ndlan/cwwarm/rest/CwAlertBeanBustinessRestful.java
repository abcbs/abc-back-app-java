...
public class CwAlertBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cw/alert/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CwAlertBean[] beans=mapper.convertValue(obj, CwAlertBean[].class);
	...
	// @Override
	public Result addOne(final CwAlertBean entity) {
	...
	// @Override
	public Result selectList(final CwAlertQuery query) {
	...
	// @Override
	public Result editOne(final CwAlertBean entity) {
	...
	public Result editSelective(final CwAlertBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
