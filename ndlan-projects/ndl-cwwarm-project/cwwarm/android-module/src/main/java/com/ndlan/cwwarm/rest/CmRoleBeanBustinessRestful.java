...
public class CmRoleBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cm/role/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CmRoleBean[] beans=mapper.convertValue(obj, CmRoleBean[].class);
	...
	// @Override
	public Result addOne(final CmRoleBean entity) {
	...
	// @Override
	public Result selectList(final CmRoleQuery query) {
	...
	// @Override
	public Result editOne(final CmRoleBean entity) {
	...
	public Result editSelective(final CmRoleBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
