...
public class CwWorkJobBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cw/work/job/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CwWorkJobBean[] beans=mapper.convertValue(obj, CwWorkJobBean[].class);
	...
	// @Override
	public Result addOne(final CwWorkJobBean entity) {
	...
	// @Override
	public Result selectList(final CwWorkJobQuery query) {
	...
	// @Override
	public Result editOne(final CwWorkJobBean entity) {
	...
	public Result editSelective(final CwWorkJobBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
