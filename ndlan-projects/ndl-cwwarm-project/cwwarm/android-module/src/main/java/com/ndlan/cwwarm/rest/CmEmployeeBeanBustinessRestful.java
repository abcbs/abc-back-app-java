...
public class CmEmployeeBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cm/employee/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CmEmployeeBean[] beans=mapper.convertValue(obj, CmEmployeeBean[].class);
	...
	// @Override
	public Result addOne(final CmEmployeeBean entity) {
	...
	// @Override
	public Result selectList(final CmEmployeeQuery query) {
	...
	// @Override
	public Result editOne(final CmEmployeeBean entity) {
	...
	public Result editSelective(final CmEmployeeBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
