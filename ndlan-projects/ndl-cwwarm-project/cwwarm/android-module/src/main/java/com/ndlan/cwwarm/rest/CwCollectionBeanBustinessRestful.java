...
public class CwCollectionBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cw/collection/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CwCollectionBean[] beans=mapper.convertValue(obj, CwCollectionBean[].class);
	...
	// @Override
	public Result addOne(final CwCollectionBean entity) {
	...
	// @Override
	public Result selectList(final CwCollectionQuery query) {
	...
	// @Override
	public Result editOne(final CwCollectionBean entity) {
	...
	public Result editSelective(final CwCollectionBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
