...
public class CmRestaurantBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cm/restaurant/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CmRestaurantBean[] beans=mapper.convertValue(obj, CmRestaurantBean[].class);
	...
	// @Override
	public Result addOne(final CmRestaurantBean entity) {
	...
	// @Override
	public Result selectList(final CmRestaurantQuery query) {
	...
	// @Override
	public Result editOne(final CmRestaurantBean entity) {
	...
	public Result editSelective(final CmRestaurantBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
