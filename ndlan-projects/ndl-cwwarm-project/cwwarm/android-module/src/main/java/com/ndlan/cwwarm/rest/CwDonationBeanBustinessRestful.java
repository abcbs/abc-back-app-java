...
public class CwDonationBeanBustinessRestful {

	private static final String url = BaseAndroidRestfulConfig.moduleServicePath+"cw/donation/";
			...
			public Result call() {
				try{
					...
					if(result.getStatus()==Result.Status.OK){
						...
						if(null!=obj){
							CwDonationBean[] beans=mapper.convertValue(obj, CwDonationBean[].class);
	...
	// @Override
	public Result addOne(final CwDonationBean entity) {
	...
	// @Override
	public Result selectList(final CwDonationQuery query) {
	...
	// @Override
	public Result editOne(final CwDonationBean entity) {
	...
	public Result editSelective(final CwDonationBean entity) {
		if(entity.getDefaultId()==null||entity.getDefaultId().trim()==""){
			return new Result(Result.Status.ERROR,"update,pk not null");
		}
		FutureTask<Result> future = new FutureTask<Result>(new Callable<Result>() {
