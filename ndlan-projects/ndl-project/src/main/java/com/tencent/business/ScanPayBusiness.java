...
public class ScanPayBusiness {
    ...
    private int waitingTimeBeforePayQueryServiceInvoked = 2000;

    //循环调用订单查询API的次数
    private int payQueryLoopInvokedCount = 30;
     ...
     * @throws Exception
     */
    public String run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        String responseString = Util.getJsonFromObject(scanPayResData);
        //异步发送统计请求
        //*
        ...
        if (scanPayResData == null || scanPayResData.getReturn_code() == null) {
            ...
            System.out.println("【支付失败】支付请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问");
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                   ...
                   boolean result=doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo,resultListener);
                	if (result) {
                        ...
                        String responseString2 = Util.getJsonFromObject(scanPayResData);
                        return responseString2;
                    } else {
                    	System.out.println("支付失败走撤销流程");
    ...
    private boolean doOnePayQuery(String outTradeNo,ResultListener resultListener) throws Exception {
        ...
        //将从API返回的Json数据映射到Java对象
        ScanPayQueryResData scanPayQueryResData=(ScanPayQueryResData) Util.getObjectFromJson("["+payQueryServiceResponseString+"]", ScanPayQueryResData.class);
//        ScanPayQueryResData scanPayQueryResData = (ScanPayQueryResData) Util.getObjectFromXML(payQueryServiceResponseString, ScanPayQueryResData.class);
        ...
        if (scanPayQueryResData.getReturn_code().equals("FAIL")) {
            ...
            /*if (!Signature.checkIsSignValidFromResponseJson(scanPayQueryResData)) {
                log.e("【支付失败】支付请求API返回的数据签名验证失败，有可能数据被篡改了");
