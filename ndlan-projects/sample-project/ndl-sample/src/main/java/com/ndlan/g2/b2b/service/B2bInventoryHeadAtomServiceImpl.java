...
public class B2bInventoryHeadAtomServiceImpl  extends BaseService<B2bInventoryHeadDao, 
	...
	SQLParam sqlParam=new SQLParam();
        ...
        String updateBy = b2bInventoryHead.getUpdateBy();
        if (StringUtils.isNotBlank(updateBy) ) {
           sqlBuffer.append("updateBy=?");
	    sqlBuffer.append(" and ");
	     param.add(b2bInventoryHead.getUpdateBy());
        }
        ...
        String status = b2bInventoryHead.getStatus();
        if (StringUtils.isNotBlank(status) ) {
           sqlBuffer.append("status=?");
	    sqlBuffer.append(" and ");
	     param.add(b2bInventoryHead.getStatus());
        }
      
            
        String createBy = b2bInventoryHead.getCreateBy();
        if (StringUtils.isNotBlank(createBy) ) {
           sqlBuffer.append("createBy=?");
	    sqlBuffer.append(" and ");
	     param.add(b2bInventoryHead.getCreateBy());
        }
	...
	Date updateTime = b2bInventoryHead.getUpdateTime();
	if (updateTime!=null  ) {
           sqlBuffer.append("updateTime=?");
	    sqlBuffer.append(" and ");
	     param.add(b2bInventoryHead.getUpdateTime());
        } 
      
            
	Date createTime = b2bInventoryHead.getCreateTime();
	if (createTime!=null  ) {
           sqlBuffer.append("createTime=?");
	    
	     param.add(b2bInventoryHead.getCreateTime());
        } 
    ...
    public B2bInventoryHeadBean trimStringValue(B2bInventoryHeadBean b2bInventoryHead) {
        ...
        if (StringUtils.isNotBlank(updateBy) && updateBy.length() > 36) {
            b2bInventoryHead.setUpdateBy(updateBy.substring(0, 36));
        }
        ...
        if (StringUtils.isNotBlank(status) && status.length() > 20) {
            b2bInventoryHead.setStatus(status.substring(0, 20));
        }

        String createBy = b2bInventoryHead.getCreateBy();
        if (StringUtils.isNotBlank(createBy) && createBy.length() > 36) {
            b2bInventoryHead.setCreateBy(createBy.substring(0, 36));
        }
