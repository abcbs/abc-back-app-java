...
import java.io.PrintStream;
 ...
 public enum InnerUserNameEnum
 {
   SUPER("super", "超级管理员"), 
   TEST("test", "测试账户"), 
   VISTER("vister", "访客"), 
   SELFORDER("selforder", "自助点餐账户");
 
   public static final String enumCode = "InnerUserName";
   public static final String enumName = "内置用户名";
   private String code;
   private String desc;
 
   private InnerUserNameEnum(String code, String desc) { this.code = code;
     this.desc = desc; }
 
   public static String getDesc(String code)
   {
     for (AuthEnum status : AuthEnum.values()) {
   ...
   public static void generateSQL()
   {
     ...
     for (AuthEnum status : AuthEnum.values()) {
       ...
       String itemSql = "insert into md_base_code_item (bci_id,bc_id,bc_code,bci_code,bci_name,bci_desc,is_enable,version) VALUES ( '" + 
         sub_uuid + "' , '" + uuid + "' ,'" + "InnerUserName" + "', '" + status.getCode() + "','" + status.getDesc() + "','" + status.getDesc() + "','1',0); ";
