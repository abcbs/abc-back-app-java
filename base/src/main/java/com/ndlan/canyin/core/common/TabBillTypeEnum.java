...
import java.io.PrintStream;
 ...
 public enum TabBillTypeEnum
 {
   ORDER("0", "普通订单"), 
   YUDING("1", "预订订单");
 
   public static final String enumCode = "TabBillType";
   public static final String enumName = "餐台订单类型";
   private String code;
   private String desc;
 
   private TabBillTypeEnum(String code, String desc) { this.code = code;
     this.desc = desc; }
 
   public static String getDesc(String code)
   {
     for (TabBillTypeEnum status : values()) {
   ...
   public static void generateSQL()
   {
     ...
     for (TabBillTypeEnum status : values()) {
       ...
       String itemSql = "insert into md_base_code_item (bci_id,bc_id,bc_code,bci_code,bci_name,bci_desc,is_enable,version) VALUES ( '" + 
         sub_uuid + "' , '" + uuid + "' ,'" + "TabBillType" + "', '" + status.getCode() + "','" + status.getDesc() + "','" + status.getDesc() + "','1',0); ";
