...
import java.io.PrintStream;
 ...
 public enum MemberCardPayTypeEnum
 {
   CONSUME("1", "使用会员卡消费"), 
   DISCOUNT("2", "使用会员卡打折");
 
   public static final String enumCode = "MembercardPayType";
   public static final String enumName = "会员卡结账类型";
   private String code;
   private String desc;
 
   private MemberCardPayTypeEnum(String code, String desc) { this.code = code;
     this.desc = desc; }
 
   public static String getDesc(String code)
   {
     for (MemberCardPayTypeEnum status : values()) {
   ...
   public static void generateSQL()
   {
     ...
     String baseSql = "insert into md_base_code (bc_id,bc_code,bc_name,bc_desc,is_enable,version) VALUES ( '" + uuid + "' , " + 
       "'" + "MembercardPayType" + "','" + "会员卡结账类型" + "','" + "会员卡结账类型" + "','1',0);";
     ...
     for (MemberCardPayTypeEnum status : values()) {
       ...
       String itemSql = "insert into md_base_code_item (bci_id,bc_id,bc_code,bci_code,bci_name,bci_desc,is_enable,version) VALUES ( '" + 
         sub_uuid + "' , '" + uuid + "' ,'" + "MembercardPayType" + "', '" + status.getCode() + "','" + status.getDesc() + "','" + status.getDesc() + "','1',0); ";
